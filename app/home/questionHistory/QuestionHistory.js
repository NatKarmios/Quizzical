...
const Cell = props => <TableCell padding="dense" {...props} />;
...
type Props = {
  ...
  loading: boolean,
  initialLoad: boolean,
  changeSortBy: string => ?mixed,
  changeSortOrder: string => ?mixed,
  changeIncludeCancelled: boolean => ?mixed,
  changeQuestionSearch: string => ?mixed,
  loadHistory: (string, string, boolean, string, number) => ?mixed
};
...
const QuestionHistory = ({
  ...
  <div style={{ padding: '30px', textAlign: 'center' }}>
    ...
    {!initialLoad || loading ? (
      <Typography>
        <i>{loading ? 'Loading...' : 'Click the submit button to search...'}</i>
      </Typography>
    ) : null}
    {initialLoad && !loading ? (
      <ResultsTable
...
const mapDispatchToProps = dispatch =>
  bindActionCreators(QuestionHistoryActions, dispatch);
